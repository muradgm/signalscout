...
const VALID_REVIEW_STATUSES = new Set(['NOT STARTED', 'IN PROGRESS', 'PASS WITH NOTES', 'COMPLETE', 'BLOCKED', 'ESCALATED']);
...
function checkRuntimeQueue() {
  const content = read('ops/runtime/PM_REVIEW_QUEUE.md');
...
function checkTaskBundles() {
  ...
  const bundles = fs.readdirSync(taskRoot, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name);
  if (bundles.length === 0) {
    warnings += 1;

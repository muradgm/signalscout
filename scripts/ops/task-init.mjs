...
if (!taskId) {
  console.error('Usage: node scripts/ops/task-init.mjs --task=AI-P1-003 [--owner=AI] [--family=audit_quality] [--gate=B] [--priority=P1]');
  process.exit(1);
}
...
for (const file of files) {
  ...
  if (fs.existsSync(file.rel)) {
    console.log(`SKIP ${path.relative(root, file.rel).replaceAll('\\', '/')}`);
    continue;
  }
  ...
  console.log(`CREATE ${path.relative(root, file.rel).replaceAll('\\', '/')}`);
}

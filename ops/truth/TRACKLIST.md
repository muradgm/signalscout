...
- district and alias handling now covers broader non-Berlin city/district cases in `tests/mvp/signals.test.mjs`
...
- bilingual mixed-copy audit friction is now explicitly differentiated in the `multilingual-local-nuremberg-langwasser` slice
...
- stored operator behavior now has a first explicit tightening pass reflected in AI generator/test work
- broader live real-case pressure now includes a multi-specialty Torhaus slice in the AI suite
- multilingual specialty send slices and the stored direct-booking thin-surface slice now have narrower, more explicit regression pressure in the AI suite
...
## Phase 7 - Lead Review Workspace `Done`
...
### Broader dashboard surface - `Done`

Evidence:
- only [LeadsPage.tsx](C:\Users\Murad\Documents\SS\signalscout\apps\dashboard\src\pages\LeadsPage.tsx) and [LeadDetailPage.tsx](C:\Users\Murad\Documents\SS\signalscout\apps\dashboard\src\pages\LeadDetailPage.tsx) remain in the dashboard page set
- sidebar exposes only the lead queue

Conclusion:
- the operator workflow is real
- the broader shell now matches the protected queue/detail wedge instead of advertising empty secondary pages
...
### 1. Expand real validation breadth for local relevance - `Next`

Do:
- build on the current non-Berlin alias and district coverage in `tests/mvp/signals.test.mjs`
- add broader real-case pressure for borderline local relevance and partial-match slices
- keep confidence and evidence quality explicit instead of letting local relevance overstate certainty
...
### 3. Continue outreach validation from the stronger benchmark base - `Next`

Do:
- keep the stricter multilingual, niche-specialty, and weak-contact outreach expectations
- add more real-case copy pressure without widening the product wedge
- keep sending secondary and replies secondary in all new slices

### 4. Validate reply flow more deeply - `Next`
...
- local relevance validation breadth: improved but still partial
- send/reply plumbing: real but not core MVP
- shell breadth: now aligned to the queue/detail wedge after removal of empty secondary pages
